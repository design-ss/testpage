...
st.write('<br>**「前後ありオーラ」「前のみ」「後ろのみ」の3種類を一気に処理はできません。** <br>アプリをリロードしてそれぞれ書き出してください。', unsafe_allow_html=True)


st.write('<br>**ID付与前に「前後オーラ」を「複数枚同時に」書き出す場合はお気をつけください。** <br> ファイルは選択順に関係なく「昇順」でアップされます。<br> そのため、適切に前後パーツを組み合わせるために、ファイル名の先頭に3桁の数字を付けるなどで順番を制御してください。', unsafe_allow_html=True)

st.write(' 例<br>前オーラ：「001.前_目玉A」「002.前_目玉B」「003.前_目玉C」<br>後ろオーラ：「004.後ろ_目玉A」「005.後ろ_目玉B」「006.後ろ_目玉C」<br> とABCそれぞれの順番が正しくなるように数字を付けてください。', unsafe_allow_html=True)
