...
# パターン1のプレビューボタンと書き出しボタン
preview_button1, export_button1, export_selected_button1 = st.columns(3)
with preview_button1:
    preview_button1 = st.button('プレビュー')

with export_button1:
    if st.button('ペット一括書き出し'):
...
# パターン1
# パターン1のプレビュー処理
if vertical_shift or horizontal_shift or scale != 0.7 or preview_button1:
    if not export_files:
        ...
        with st.spinner("プレビュー画像生成中です..."):
            ...
            for i, export_file in enumerate(export_files):
                ...
                # サンプルフレームを読み込む
                flame_image = Image.open("100_flame.png")
                ...
                if cols[i % 4].checkbox(f" {export_file.name}", key=f"select_{export_file.name}"):
